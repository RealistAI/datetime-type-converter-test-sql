...
CSV_INPUT_FOLDER = Path(Path.cwd(), 'input_csv')
...
CASETYPE_CONVERTER = Path(Path.cwd(), "template_configs", "column-case-converter.config.yaml")

SQL_TO_TRANSLATE = Path(Path.cwd(), "translate.sql")
OBJECT_CSV_FILE = Path(CSV_INPUT_FOLDER, "schema_map.csv")


COLUMN_CSV_FILE = Path(CSV_INPUT_FOLDER, "columns.csv")
